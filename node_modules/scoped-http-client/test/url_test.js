...
(function() {
  var ScopedClient, Sys, assert, called, client;
  ...
  client.query('a').host('bar.com').port(443).query('b', 2).query({
    c: 3
  }).path('/boom');
  ...
  client.scope('api', function(scope) {
    called = true;
    return assert.equal('/boom/api', scope.options.pathname);
  });
  ...
  client.scope('http://', function(scope) {
    ...
    return assert.equal('/boom', scope.options.pathname);
  });
  ...
  client.scope('https://bar.com', function(scope) {
    ...
    return assert.equal('/', scope.options.pathname);
  });
  ...
  client.scope('/help', {
    ...
    assert.equal('http:', scope.options.protocol);
    assert.equal('foo.com', scope.options.hostname);
    return assert.equal('/help', scope.options.pathname);
  });
